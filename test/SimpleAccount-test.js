...
describe("SimpleAccount Test", function () {
  before(async () => {
    ...
    // SimpleAccount 컨트랙트
    const simpleAccount = await hre.ethers.getContractFactory(
      "SimpleAccount",
      signers[0]
    );
    owner = await signers[0].getAddress(); // signer[0] is owner
    proxy = await hre.upgrades.deployProxy(simpleAccount, [owner], {
      kind: "uups",
  ...
  it("Confirm proxy name", async () => {
    assert.equal("SimpleAccount", await proxy.CONTRACTNAME());
  });

  it("Confirm the implementation address", async () => {
    const impl = await getImplementationAddress(provider, proxy.address);
    const impl_slot = await provider.getStorageAt(
      proxy.address,
      IMPLEMENTATION_SLOT
    );
    expect(impl).to.be.equal(
      hre.ethers.utils.getAddress(hre.ethers.utils.hexStripZeros(impl_slot))
    );
  });
  ...
  it("Wallet owner should be able to send ETH", async () => {
    ...
    // proxy balance = 5 - 1 = 4
    assert.equal(
      hre.ethers.utils.parseEther("4").toString(),
      balance.toString()
    );
    // recipientBalanceBefore + 1 = recipientBalanceAfter
    assert.equal(
      recipientBalanceBefore.add(hre.ethers.utils.parseEther("1")).toString(),
  ...
  it("Wallet should be able to deposit ETH to EntryPoint", async () => {
    ...
    const balanceEntryPointBefore = await provider.getBalance(
      entryPoint.address
    );
    await proxy.addDeposit({ value: deposit });
    const balanceEntryPointAfter = await provider.getBalance(
      entryPoint.address
    );
    // check balance of entryPoint = balanceEntryPointBefore + deposit
    assert.equal(
      balanceEntryPointBefore.add(deposit).toString(),
      balanceEntryPointAfter.toString()
    );
  ...
  it("Wallet should be able to call handleOps", async () => {
    const { message, userOp } = getUserOpHash(
      proxy.address,
      0,
      entryPoint.address,
      chainId
    ...
    await expect(
      signers[5].sendTransaction({ to: entryPoint.address, data })
